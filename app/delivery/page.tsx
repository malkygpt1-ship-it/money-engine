...
export default async function DeliveryPage({searchParams}:{searchParams:Promise<{session_id?:string}>}){
 ...
 await db.from("revenue_events").upsert({opportunity_id:opportunityId,provider:"stripe",external_id:session.payment_intent||session.id,gross_gbp:amount,fee_gbp:0,occurred_at:new Date().toISOString(),metadata:{checkout_session:session.id}},{onConflict:"provider,external_id"});
